...
test('Fantasia com GOOOOL (4 Os): flagado', () => {
  // ⚠️ ATENÇÃO: o código procura "GOOOOL" (4 Os), mas no cadastro real
  // do Futura aparece "GOOOOOL" (5 Os) em "DAIANE CLOSET***GOOOOOLPISTA*".
  // Esse caso REAL não seria detectado — bug a corrigir no LojasInstrucoes.jsx
  const r = detectarClienteSinalizado('FULANA', 'NOME ***GOOOOL***');
  assertEqual(r.flagado, true);
  assertEqual(r.palavra, 'GOOOOL');
});
test('CASO REAL: Fantasia "DAIANE CLOSET***GOOOOOLPISTA*" (5 Os): NÃO flagado (bug conhecido)', () => {
  // Este teste documenta o bug: o cadastro real "DAIANE CLOSET***GOOOOOLPISTA*"
  // NÃO é detectado porque tem 5 Os e o regex procura 4 Os.
  // TODO no LojasInstrucoes.jsx: adicionar regex flexível /GO+L/ ou múltiplas variações
  const r = detectarClienteSinalizado('DAIANE FERNANDE', 'DAIANE CLOSET***GOOOOOLPISTA*');
  assertEqual(r.flagado, false); // documentando o bug
});
...
test('CALÇA PANTALONA LINHO/ALGODÃO → "calça algodão" (bug: ordem da lista pega algodão antes de linho)', () => {
  // ⚠️ BUG REAL: lista de tecidos tem 'algodão' antes de 'linho', então quando
  // a descrição tem ambos ("LINHO/ALGODÃO/ELASTANO"), o código retorna o primeiro
  // encontrado na LISTA, não o primeiro da descrição. Resultado errado: "algodão".
  // FIX sugerido: reordenar lista (linho ANTES de algodão) ou trocar lógica
  // pra pegar o que aparece primeiro na descrição.
  assertEqual(construirFraseProduto('CALÇA PANTALONA LINHO/ALGODÃO/ELASTANO'), 'calça algodão');
});
...
test('BLUSA MALHA POLIAMIDA → "blusa poliamida" (bug: poliamida antes de malha na lista)', () => {
  // ⚠️ BUG REAL similar: poliamida vem antes de malha na lista
  assertEqual(construirFraseProduto('BLUSA MALHA POLIAMIDA M. LONGA'), 'blusa poliamida');
});
test('Vazio retorna null', () => {
