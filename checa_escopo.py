...
for (comp, nome), linha in uso.items():
    daFuncao = any(nome in src[i] for i, c in comps if c == comp)   # prop do componente
    daFuncao = daFuncao or any(nome in l and ('=>' in l or 'function' in l) and '{' in l
                               for l in src[max(0, linha - 40):linha])
