  ...
  useEffect(()=>{
    ...
    setCortesSala(prev=>{
      ...
      const att=prev.map(c=>{
        ...
        const p=produtos.find(x=>String(x.ref).replace(/^0+/,'')===String(c.ref).replace(/^0+/,''));
        if(!p)return c;
        mudou=true;
