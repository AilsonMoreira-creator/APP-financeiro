...
// FIX: DADOS_MENSAIS com todos os campos completos
const DADOS_MENSAIS = {
        ...
        <>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:16,marginBottom:16}}>
            ...
            <div style={{background:"#fff",borderRadius:12,padding:22,border:"1px solid #e8e2da"}}>
              ...
              {varD&&<div style={{fontSize:12,color:Number(varD)<=0?"#27ae60":"#c0392b"}}>{Number(varD)>=0?"+":""}{varD}% vs mês ant.</div>}
            </div>
  ...
  return(
    <div style={{borderBottom:"1px solid #e8e2da"}}>
      <div onClick={()=>setAberto(p=>!p)} style={{padding:"9px 16px",background:"#f7f4f0",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span style={{fontSize:11,color:"#a89f94",letterSpacing:2,textTransform:"uppercase"}}>Gerenciar Prestadores</span>
        <span style={{fontSize:12,color:"#a3bacc"}}>{aberto?"▲":"▼"}</span>
      </div>
      {aberto&&(
            ...
            <div key={idx} style={rowStyle}>
              ...
              {isTecidos?(
                <>
                  ...
                  <div style={{padding:"6px 8px"}}><input value={row.nroNota||""} onChange={e=>updateLinhaAux(auxAberta,idx,"nroNota",e.target.value)} style={fromBoleto?dis:inputStyle} placeholder="NF-001" readOnly={fromBoleto}/></div>
                  <div style={{padding:"6px 8px"}}><input value={row.valor||""} onChange={e=>updateLinhaAux(auxAberta,idx,"valor",e.target.value)} style={{...(fromBoleto?dis:inputStyle),textAlign:"right"}} placeholder="0,00" readOnly={fromBoleto}/></div>
                </>
              ):temPrest?(
                <>
...
const AGENDA_INICIAL=[
  ...
  {id:27,dia:28,descricao:"Boa Vista",feito:false},{id:28,dia:28,descricao:"Metromed",feito:false},
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:8}}>
        {[{label:"Vencidos",value:alertas.length,color:alertas.length>0?"#c0392b":"#a89f94",bg:alertas.length>0?"#fdeaea":"#fff"},{label:"Hoje",value:hojeItems.length,color:"#4a7fa5",bg:"#f0f6fb"},{label:"Pendentes",value:proximos.length,color:"#2c3e50",bg:"#fff"}].map(c=>(
    ...
    return(
      <div>
        ...
        <div style={{background:"#fff",borderRadius:12,border:"1px solid #e8e2da",overflow:"hidden"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead>
              <tr style={{background:"#f7f4f0",borderBottom:"2px solid #e8e2da"}}>
                <th style={{padding:"10px 16px",textAlign:"left",fontSize:11,color:"#a89f94",fontWeight:600}}>Categoria</th>
...
const OficinasContent=({cortes,setCortes,produtos,setProdutos,oficinasCAD,setOficinasCAD,logTroca,setLogTroca,setAuxDataPorMes,mesAtual})=>{
  ...
  const executarTroca=()=>{
    ...
    setTrocaMsg(`✓ REF ${trocaDe} → ${trocaPara} atualizada em tudo.`);
  ...
  const alertas=oficinasUnicas.map(of=>{
    ...
    const pct=Math.round((aberto-med)/med*100);
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",borderBottom:"1px solid #e8e2da",marginBottom:16}}>
        ...
        <TabBtn id="cadastros" label="Cadastros"  icon="🗂️"/>
      </div>
      ...
      {aba==="dashboard"&&(()=>{
        ...
        const perda=totalEnviadas>0?totalEnviadas-totalEntregues:0;
        ...
        return(
          <div>
            ...
            <div style={{marginBottom:16}}>
              <button onClick={()=>setVerValores(p=>!p)} style={{background:"#fff",border:"1px solid #e8e2da",borderRadius:8,padding:"7px 16px",fontSize:11,cursor:"pointer",color:"#6b7c8a",fontFamily:"Georgia,serif",display:"flex",alignItems:"center",gap:8}}>
                💰 {verValores?"Ocultar valores financeiros":"Ver valores financeiros"}
      ...
      <Section title="Regras de Dedução — Marketplaces" subtitle="Aplicadas automaticamente sobre receita bruta">
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
          <div>
            ...
            <div style={{fontSize:11,color:"#a89f94",marginBottom:8}}>Deduzido do valor bruto de marketplaces</div>
            <div style={{display:"flex",alignItems:"center",gap:8}}><input value={regras.devolucao} onChange={e=>setRegras(p=>({...p,devolucao:e.target.value}))} style={{...iStyle,flex:"unset",width:60}}/><span style={{fontSize:14,color:"#6b7c8a",fontWeight:600}}>%</span><span style={{fontSize:11,color:"#a89f94"}}>(padrão: 10%)</span></div>
          </div>
        </div>
      </Section>
      ...
      <div style={{background:"#fff",borderRadius:12,border:"1px solid #e8e2da",overflow:"hidden",marginTop:16}}>
        ...
        {verCodigo&&(
          <div style={{padding:16}}>
            ...
            <div style={{fontSize:11,color:"#a89f94",marginBottom:8}}>
  ...
  const dadosMensais = Object.fromEntries(
    Array.from({length:12},(_,i)=>{
      ...
      if(mesNum < MES_ATUAL){
        ...
        return [i, {receita:0,despesa:0,silvaTeles:0,bomRetiro:0,marketplaces:0,prolabore:0,oficinas:0,tecidos:0}];
      }
    })
  );
