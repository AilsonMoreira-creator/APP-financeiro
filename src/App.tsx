        ...
        <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:14,width:"100%",maxWidth:680,overflow:"hidden",boxShadow:"0 20px 50px rgba(0,0,0,0.25)"}}>
          ...
          <div style={{padding:"16px 20px"}}>
            <div style={{display:"flex",gap:8,marginBottom:14}}>
              ...
              <button onClick={()=>setSaleOpen({ref:modalRef,desc})} title={vermelho?`REF ${modalRef}: promoção ATIVA acima do teto (7% / 12%) — abra e confira`:verde?`Tem promoção do Mercado Livre dentro da faixa da REF ${modalRef}`:`Promoções do Mercado Livre pra REF ${modalRef}`} style={{flex:mobile?"1 1 calc(50% - 4px)":"1 1 22%",minWidth:0,whiteSpace:"nowrap",background:vermelho?"#fdecea":verde?"#e9f5ee":"#fff",color:vermelho?"#a33":verde?"#1f7a48":"#2c3e50",border:`1px solid ${vermelho?"#e3a3a3":verde?"#7fc79b":"#c8d8e4"}`,borderRadius:8,padding:mobile?"11px 6px":"9px 8px",fontSize:mobile?11.5:12,fontWeight:700,fontFamily:"Georgia,serif",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:5,boxShadow:vermelho?"0 0 0 2px rgba(170,51,51,.18)":verde?"0 0 0 2px rgba(31,122,72,.15)":"none"}}>
...
const CreditoAnthropic=()=>{
  ...
  return(<div style={{background:"#faf8f4",borderRadius:10,padding:"10px 12px",marginBottom:14}}>
    ...
    <div style={{fontSize:12,color:"#6b6259",marginTop:6}}>gasto 7 dias <b>US$ {c.gasto_7d.toFixed(2)}</b> · 30 dias <b>US$ {c.gasto_30d.toFixed(2)}</b> · média <b>US$ {c.media_dia.toFixed(2)}/dia</b>{c.dias_restantes!=null?<> · dá pra <b>{c.dias_restantes} dias</b></>:null}{c.fonte&&<span style={{color:"#a89f94"}}> · fonte: {c.fonte}</span>}{c.ultimo_custo_dia?<span style={{color:"#a89f94"}}> · último dia lido {c.ultimo_custo_dia.dia.split("-").reverse().join("/")}</span>:null}</div>
