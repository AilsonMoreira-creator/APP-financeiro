    ...
    supabase.from('amicia_data').select('payload').eq('user_id','calc-meluni').single()
      .then(({data})=>{
        if(data?.payload){
          ...
          try{localStorage.setItem("amica_calc",JSON.stringify({prods:data.payload.prods||[],prs:data.payload.prs||{}}));}catch(e){}
        }
        setSyncStatus('saved');
...
var _S="#2c3e50",_B="#4a7fa5",_BG="#f7f4f0",_BD="#e8e2da",_W="#fff",_GR="#27ae60",_RD="#c0392b",_GO="#c8a040";
var _FN="Calibri,'Segoe UI',Arial,sans-serif",_GE="Georgia,serif";
...
var fmt=function(v){return(v||0).toLocaleString("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2});};
var fmtR=function(v){return "R$ "+fmt(v);};
...
var btnPri={background:_B,color:_W,border:"none",borderRadius:8,padding:"8px 14px",fontSize:12,cursor:"pointer",fontFamily:_GE,fontWeight:600};
...
var btnVolt={background:_W,border:"1px solid "+_BD,borderRadius:8,padding:"7px 14px",cursor:"pointer",fontSize:13,color:_B,fontFamily:_GE};
...
function SvgFichaTecnica(props){
    ...
    <div style={{background:_BG,minHeight:"100vh",padding:20,fontFamily:_GE}}>
      <div style={{maxWidth:800,margin:"0 auto"}}>
        ...
        <div style={{background:_W,borderRadius:12,border:"1px solid "+_BD,overflow:"hidden"}}>
          <div style={{display:"grid",gridTemplateColumns:"70px 1fr 80px 120px 90px",background:_B}}>
          ...
          {ord.map(function(p,i){return(
            <div key={p.id} onClick={function(){onSelect(p);}}
              ...
              <div style={{padding:"10px 12px",fontSize:12,fontWeight:700,color:_B}}>{p.ref}</div>
    ...
    <div style={{background:_BG,minHeight:"100vh",padding:20,fontFamily:_GE}}>
      <div style={{maxWidth:740,margin:"0 auto"}}>
        ...
        <div style={{background:_W,borderRadius:12,padding:20,border:"1px solid "+_BD}}>
          ...
          <div style={{borderTop:"2px solid "+_B,marginBottom:14,paddingTop:14}}>
            <div style={{fontSize:12,fontWeight:700,color:_B,letterSpacing:1,textTransform:"uppercase",marginBottom:12}}>Componentes de Custo</div>
          </div>
          {/* Rows: 4+4 campos */}
      ...
      <div style={{maxWidth:860,margin:"0 auto"}}>
        ...
        {tab==="detalhe"&&prod&&(<div>
          <div style={{background:_W,borderRadius:12,padding:14,border:"1px solid "+_BD,marginBottom:14,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            {!editMode&&<button onClick={iniciarEdicao} style={{background:_W,border:"1px solid #c8d8e4",borderRadius:6,padding:"5px 12px",fontSize:12,cursor:"pointer",color:_B}}>✏ Editar</button>}
          </div>
          ...
          <div style={{background:_W,borderRadius:12,padding:16,border:"1px solid "+_BD,marginBottom:14}}>
            <div style={{fontSize:12,fontWeight:600,color:_B,marginBottom:12}}>Composição de Custo</div>
          ...
          <div style={{background:_W,borderRadius:12,border:"1px solid "+_BD,marginBottom:14,overflow:"hidden"}}>
            <div style={{padding:"12px 16px",fontSize:12,fontWeight:600,color:_B}}>Análise de Preços</div>
...
const FichaTecnicaContent=()=>{
  ...
  var _sync=useState(null),syncStatusFT=_sync[0],setSyncStatusFT=_sync[1];
  ...
  useEffect(function(){
    ...
    // Camada 2: Supabase
    if(!supabase)return;
    setSyncStatusFT('saving');
    supabase.from('amicia_data').select('payload').eq('user_id','ficha-tecnica').single()
      .then(function(res){var data=res.data;
        if(data&&data.payload){
          if(data.payload.produtos){setProdutos(data.payload.produtos);produtosRef.current=data.payload.produtos;}
          if(data.payload.precosSalvos){setPrecosSalvos(data.payload.precosSalvos);precosRef.current=data.payload.precosSalvos;}
          try{localStorage.setItem("amica_ficha",JSON.stringify({produtos:data.payload.produtos||[],precosSalvos:data.payload.precosSalvos||{}}));}catch(e){}
        }
        setSyncStatusFT('saved');setTimeout(function(){setSyncStatusFT(null);},2000);
      }).catch(function(){setSyncStatusFT('error');});
  },[]);

  var salvarLocal=useCallback(function(pr,pc){try{localStorage.setItem("amica_ficha",JSON.stringify({produtos:pr,precosSalvos:pc}));}catch(e){}},[]);
  var salvarSupabase=useCallback(function(pr,pc){
    if(!supabase)return;
    setSyncStatusFT('saving');
    supabase.from('amicia_data').upsert({user_id:'ficha-tecnica',payload:{produtos:pr,precosSalvos:pc}},{onConflict:'user_id'})
      .then(function(){setSyncStatusFT('saved');setTimeout(function(){setSyncStatusFT(null);},2000);})
      .catch(function(){setSyncStatusFT('error');});
  },[]);
  var atualizarProdutos=useCallback(function(fn){var n=typeof fn==="function"?fn(produtosRef.current):fn;produtosRef.current=n;setProdutos(n);salvarLocal(n,precosRef.current);salvarSupabase(n,precosRef.current);},[salvarLocal,salvarSupabase]);
  var atualizarPrecos=useCallback(function(fn){var n=typeof fn==="function"?fn(precosRef.current):fn;precosRef.current=n;setPrecosSalvos(n);salvarLocal(produtosRef.current,n);salvarSupabase(produtosRef.current,n);},[salvarLocal,salvarSupabase]);
    ...
    <div style={{background:_BG,minHeight:"100vh",padding:20,fontFamily:_GE}}>
      <div style={{maxWidth:980,margin:"0 auto"}}>
        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:16}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            {syncStatusFT==='saving'&&<span style={{fontSize:11,color:"#a89f94",fontFamily:"Georgia,serif"}}>⏳ Salvando...</span>}
            {syncStatusFT==='saved'&&<span style={{fontSize:11,color:"#27ae60",fontFamily:"Georgia,serif"}}>✓ Salvo</span>}
            {syncStatusFT==='error'&&<span style={{fontSize:11,color:"#c0392b",fontFamily:"Georgia,serif"}}>⚠ Erro ao salvar</span>}
...
const FichaTecnicaContent=()=>{
  ...
  // Camada 1: localStorage imediato + Camada 2: Supabase
  useEffect(function(){
    try{var raw=localStorage.getItem("amica_ficha");if(raw){var d=JSON.parse(raw);
      if(d.produtos){setProdutos(d.produtos);produtosRef.current=d.produtos;}
      if(d.precosSalvos){setPrecosSalvos(d.precosSalvos);precosRef.current=d.precosSalvos;}
    ...
    supabase.from('amicia_data').select('payload').eq('user_id','ficha-tecnica').single()
      .then(function(r){var d=r.data;
        if(d&&d.payload){
          ...
          try{localStorage.setItem("amica_ficha",JSON.stringify({produtos:d.payload.produtos||[],precosSalvos:d.payload.precosSalvos||{}}));}catch(e){}
        }
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    // Camada 1: carrega localStorage imediatamente
    try{
        ...
        <div style={{display:"flex",gap:1,overflowX:"auto",flex:1,scrollbarWidth:"none",msOverflowStyle:"none"}}>
          {modulosVisiveis.map(m=>(
            <button key={m.id} onClick={()=>{
                // Salva imediatamente ao trocar de módulo
                if(debounceRef.current)clearTimeout(debounceRef.current);
                salvarNoSupabase({receitasPorMes,auxDataPorMes,categoriasPorMes,boletosShared,cortes,produtos,oficinasCAD,logTroca,usuarios,prestadores});
                setActive(m.id);
              }}
        ...
        <div style={{position:"relative",flexShrink:0,display:"flex",alignItems:"center",gap:8}} id="user-menu">
          {syncStatus&&(
            <span style={{fontSize:11,padding:"3px 9px",borderRadius:10,
              background:syncStatus==='loading'||syncStatus==='saving'?"#f0f6fb":syncStatus==='saved'?"#eafbf0":"#fdeaea",
              color:syncStatus==='loading'||syncStatus==='saving'?"#4a7fa5":syncStatus==='saved'?"#27ae60":"#c0392b",
              fontFamily:"Georgia,serif",whiteSpace:"nowrap"}}>
              {syncStatus==='loading'?"⏳ carregando…":syncStatus==='saving'?"☁ salvando…":syncStatus==='saved'?"☁ salvo":"✗ erro sync"}
            </span>
          )}
