...
const BlingContent=({setReceitasMes,mesAtual,blingVendas={},blingImportStatus=null,produtos=[]})=>{
  ...
  // Carrega fotos dos produtos da Calculadora (calc-meluni)
  const [calcProds,setCalcProds]=useState([]);
  useEffect(()=>{
    try{const d=JSON.parse(localStorage.getItem("amica_calc")||"{}");if(d?.prods)setCalcProds(d.prods);}catch{}
    if(supabase){supabase.from('amicia_data').select('payload').eq('user_id','calc-meluni').single().then(({data})=>{if(data?.payload?.prods)setCalcProds(data.payload.prods);}).catch(()=>{});}
  },[]);
  // Merge: procura foto primeiro no calcProds, depois no produtos
  const findFoto=(ref)=>{const normRef=(r)=>String(r).replace(/^0+/,'');const nr=normRef(ref);return calcProds.find(x=>normRef(x.ref)===nr)?.foto||produtos.find(x=>normRef(x.ref)===nr)?.foto||null;};
  ...
  useEffect(()=>{
    ...
    setCortesSala(prev=>{
      ...
      const att=prev.map(c=>{
        ...
        const normR=(r)=>String(r).replace(/^0+/,'');const p=calcProds.find(x=>normR(x.ref)===normR(c.ref))||produtos.find(x=>normR(x.ref)===normR(c.ref));
        if(!p)return c;
        mudou=true;
