...
export default async function handler(req, res) {
  ...
  const cards = [
    ...
    ['Valor real (vendas − devolução)', brl(d.valor_real), '', true],
  ...
  const gap = 10, cw = (W - 2 * M - gap) / 2, ch = 42;
  cards.forEach((c, i) => {
    const col = i % 2, row = Math.floor(i / 2);
    const x = M + col * (cw + gap), yy = y + row * (ch + 7);
    ...
    doc.font('Helvetica-Bold').fontSize(9.5).fillColor('#4a5a68').text(label, x + 10, yy + (sub ? 8 : 15), { width: cw - 110, lineBreak: false });
    if (sub) doc.font('Helvetica').fontSize(8.5).fillColor(MUTED).text(sub, x + 10, yy + 22, { width: cw - 110, lineBreak: false });
    doc.font('Helvetica-Bold').fontSize(15).fillColor(INK).text(valor, x + cw - 110, yy + 12, { width: 100, align: 'right', lineBreak: false });
  });
  y += Math.ceil(cards.length / 2) * (ch + 7) + 10;

  // dia a dia
  doc.font('Helvetica-Bold').fontSize(12).fillColor(ROXO).text('Dia a dia', M, y);
  y += 18;
  const cols = [
  ...
  for (const s of serie) {
    ...
    vals.forEach((v, i) => { doc.text(v, x, y + 3, { width: cols[i].w - 6, align: cols[i].r ? 'right' : 'left', lineBreak: false }); x += cols[i].w; });
    const bw = cols[5].w - 8, bh = 7, frac = n(s.vendas_valor) / maxV;
    doc.roundedRect(x, y + 5, bw, bh, 3).fillColor('#ece3f2').fill();
    if (frac > 0) doc.roundedRect(x, y + 5, Math.max(4, bw * frac), bh, 3).fillColor(ROXO).fill();
    y += rowH;
