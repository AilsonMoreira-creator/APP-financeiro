  ...
  try {
    ...
    if (req.query?.etiqueta_pedido) {
      ...
      if (link) {
        ...
        if (out.arquivo.tipo !== 'pdf' && out.arquivo.tipo !== 'zip') out.arquivo.inicio_texto = Buffer.from(b.slice(0, req.query?.tudo ? b.length : 700)).toString('latin1');
    ...
    if (req.query?.etiqueta === '1' && req.query?.pedido_id) {
      ...
      if (vol) {
        // por objeto, nos 2 formatos: query string e GET com BODY (schema historico)
        ...
        await new Promise(r => setTimeout(r, 700));  // Bling: 3 req/s
