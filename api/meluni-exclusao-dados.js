// Página/Callback de Exclusão de Dados do Usuário (campo exigido pela Meta).
// GET  -> HTML com instruções de exclusão.
// POST -> trata o signed_request da Meta e devolve {url, confirmation_code}.
// Loga método/corpo pra diagnóstico (runtime logs do Vercel).
...
export default async function handler(req, res) {
  try {
    ...
    console.log(`[exclusao] ${req.method} ct=${ct} bodyType=${typeof req.body} keys=${req.body && typeof req.body === 'object' ? Object.keys(req.body).join(',') : 'n/a'}`);
    ...
    if (req.method === 'POST') {
      ...
      console.log(`[exclusao] POST signedPresent=${!!signed} userId=${userId} -> code=${code}`);
...
<li>Informe o <strong>@ do seu Instagram</strong> ou o <strong>telefone do WhatsApp</strong> usado no contato.</li>
</ol>
<p>Concluímos em até <strong>15 dias</strong>, salvo dados que a lei exige manter.</p>
${code ? `<p>Código da solicitação: <code>${code}</code></p>` : ''}
<p>Detalhes na <a href="https://meluniloja.com.br/pagina/privacidade">Política de Privacidade</a>.</p>
</body></html>`);
  } catch (e) {
    console.error('[exclusao] erro:', e && e.message || e);
    res.status(200).json({ url: `${BASE}/api/meluni-exclusao-dados`, confirmation_code: 'del_fallback' });
  }
}
