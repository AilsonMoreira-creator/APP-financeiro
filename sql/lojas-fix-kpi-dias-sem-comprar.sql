-- ═══════════════════════════════════════════════════════════════════════════
-- MIGRATION: Fix bug NULL em dias_sem_comprar / fase_ciclo_vida / status_atual
-- Data: 2026-04-28
-- ═══════════════════════════════════════════════════════════════════════════
--
-- BUG: A função lojas_recalcular_kpis_cliente tinha:
--      v_dias_sem := EXTRACT(DAY FROM CURRENT_DATE - v_ultima)::int
--
--      Em Postgres, "date - date" já retorna integer de dias diretamente.
--      EXTRACT(DAY FROM <integer>) retorna NULL silencioso (sem erro).
--      Resultado: 727 clientes com ultima_compra/qtd_compras OK mas
--      dias_sem_comprar/status_atual/fase_ciclo_vida todos NULL.
--
--      Caso real que detectou: cliente CLAMARROCA com lifetime R$ 192k,
--      44 compras, última em 2026-04-24 (5 dias atrás). KPI mostrava
--      dias_sem_comprar=NULL e a IA tentava classificá-la como
--      "reativar" inventando justificativa.
--
-- FIX: Subtração direta + cast: (CURRENT_DATE - v_ultima)::int
--
-- COMO APLICAR:
--   1. Cola este arquivo INTEIRO no Supabase SQL Editor
--   2. Run
--   3. Roda em seguida: SELECT lojas_recalcular_kpis_todos();
--      (recalcula KPIs de todos os clientes pra propagar o fix)
--
...
BEGIN
  ...
  -- ✅ FIX: subtração direta retorna int em dias
...
-- ═══════════════════════════════════════════════════════════════════════════
-- SMOKE TEST (rodar depois)
-- ═══════════════════════════════════════════════════════════════════════════
-- Recalcular todos pra propagar o fix:
--   SELECT lojas_recalcular_kpis_todos();
--
-- Confirmar que NÃO sobrou ninguém com bug:
--   SELECT COUNT(*) as ainda_bugados
--   FROM lojas_clientes_kpis
--   WHERE qtd_compras > 0 AND dias_sem_comprar IS NULL;
--   (deve retornar 0)
-- ═══════════════════════════════════════════════════════════════════════════
